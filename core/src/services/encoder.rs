...
mod tests {
    ...
    fn test_webp_encoding() {
        ...
        for quality in [10, 50, 90] {
            ...
            assert!(result.is_ok(), "Failed to encode WebP at quality {}", quality);
            
            let encoded = result.unwrap();
            assert!(!encoded.is_empty(), "WebP encoded output is empty at quality {}", quality);
            ...
            assert_eq!(decoded.width() as u32, image.width(), "Width mismatch at quality {}", quality);
            assert_eq!(decoded.height() as u32, image.height(), "Height mismatch at quality {}", quality);
        }
    }
    ...
    fn test_jpeg_encoding() {
        ...
        let high_quality = encoder.encode(&image, &create_test_options(Some(90))).unwrap();
        let low_quality = encoder.encode(&image, &create_test_options(Some(10))).unwrap();
